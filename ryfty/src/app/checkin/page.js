...
export default function CheckinPage() {
  ...
  const processReservation = async (reservationId) => {
    try {
      ...
      } else if (error.message.includes('Missing or invalid authorization header')) {
        errorMessage = 'Device authorization expired. Please re-scan the device QR code.';
      } else if (error.message.includes('Invalid or expired token')) {
        errorMessage = 'Device authorization expired. Please re-scan the device QR code.';
      } else if (error.message.includes('Reservation ID is required')) {
        errorMessage = 'Invalid QR code format. Please scan a valid reservation QR code.';
      } else if (error.message.includes('Reservation not found in memory')) {
        errorMessage = 'Reservation not found. This QR code may be invalid or expired.';
      } else if (error.message.includes('Already checked in')) {
        errorMessage = 'This customer has already been checked in.';
      } else if (error.message.includes('Device authorization expired')) {
        errorMessage = 'Device authorization expired. Please re-scan the device QR code.';
      } else if (error.message.includes('Invalid check-in request')) {
        errorMessage = 'Invalid check-in request. Please try again.';
      } else if (error.message.includes('Check-in failed')) {
        errorMessage = error.message; // Use the full error message from API
      } else {
        // For any other errors, use the error message directly if it's meaningful
        if (error.message && error.message.length > 0 && !error.message.includes('API call failed')) {
    ...
    <div className="checkin-mobile-page">
      <div className="checkin-camera-container">
        ...
        <div className="camera-view">
          ...
          {/* Scan Error */}
          {scanError && (
            <div className={`scan-error ${scanError.includes('authorization') || scanError.includes('expired') ? 'critical' : ''}`}>
              <div className="error-icon">
                {scanError.includes('authorization') || scanError.includes('expired') ? (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              <p>{scanError}</p>
              {scanError.includes('authorization') || scanError.includes('expired') && (
                <small>Please contact your administrator to re-authorize this device</small>
              )}
            </div>
          )}
