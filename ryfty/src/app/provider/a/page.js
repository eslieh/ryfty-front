...
export default function ProviderProfile() {
  ...
  const handleAvatarUpload = async (event) => {
    ...
    if (!config.upload.allowedTypes.includes(file.type)) {
      alert(`Please select a valid image file. Allowed types: ${config.upload.allowedTypes.join(', ')}`);
      return;
    }

    // Validate file size
    if (file.size > config.upload.maxFileSize) {
      alert(`File size must be less than ${Math.round(config.upload.maxFileSize / (1024 * 1024))}MB`);
      return;
    }
    ...
    try {
      ...
      // Transformations should be configured in the upload preset or applied via URL
